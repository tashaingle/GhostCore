...
export async function GET(request:Request) {
  const url=new URL(request.url); const code=url.searchParams.get("code");const requested=url.searchParams.get("next")??"/app/command-centre";const next=requested.startsWith("/")&&!requested.startsWith("//")?requested:"/app/command-centre";
